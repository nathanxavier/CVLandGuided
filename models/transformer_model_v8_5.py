  ...
  def forward(self, id, y, y1=None, seg1=None, y_cls=None):
    if(id=="aerial"):
      ...
      if "featup" in self.base:
        ...
        aerial_set = self.aerial_feat3(y1[3])
        street_set = self.x_cls3(self.street_feat3(y[3]))
        cos_sim = self.cos(aerial_set, street_set)
        map = self.depth_3(cos_sim.unsqueeze(1))
        map = self.up(map)

        aerial_set = self.aerial_feat2(y1[2])
        street_set = self.x_cls2(self.street_feat2(y[2]))
        cos_sim = self.cos(aerial_set, street_set)
        map = torch.cat([map, cos_sim.unsqueeze(1)], dim=1)
        map = self.depth_2(map)
        map = self.up(map)

        aerial_set = self.aerial_feat1(y1[1])
        street_set = self.x_cls1(self.street_feat1(y[1]))
        cos_sim = self.cos(aerial_set, street_set)
        map = torch.cat([map, cos_sim.unsqueeze(1)], dim=1)
        map = self.depth_1(map)
        map = self.up(map)

        aerial_set = self.aerial_feat0(y1[0])
        street_set = self.x_cls0(self.street_feat0(y[0]))
        cos_sim = self.cos(aerial_set, street_set)
        map = torch.cat([map, cos_sim.unsqueeze(1)], dim=1)
...
class GlobalClassBlock(nn.Module):
  def __init__(self, aerial_embed_dim, street_embed_dim, input_dim, droprate=.5, ReLU=False, bnorm=True, num_bottleneck=1024, linear=True, return_f = False):
